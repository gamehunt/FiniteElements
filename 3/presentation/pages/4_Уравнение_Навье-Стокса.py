...
if menu == "Уравнения Навье-Стокса":
    ...
    **Стационарная форма** ($\partial / \partial t = 0$, $f = 0$):
    $$
    (\bm{u} \cdot \nabla) \bm{u} = - \operatorname{grad}p + \nu \operatorname{div} \operatorname{grad} \bm{u}
    $$
    ...
    **Вариационная постановка**:
    ...
    $$
    \bm{V} - \text{пространство достаточно гладких функций}
    $$
    $$
    \bm{V_D} = \{ \bm{v} \in V^2 : \bm{v}(x) = g(x), \ x \in \partial \Omega \}
    $$
    $$
    \bm{V_0} = \{ \bm{v} \in V^2 : \bm{v}(x) = 0, \ x \in \partial \Omega \}
    $$

    Для давления:

    $$
    Q - \text{пространство достаточно гладких функций}
    $$

    $$
    \int_{\Omega} (\bm{u} \cdot \nabla)\bm{u} \cdot \bm{v}\,dx
    + \nu \int_{\Omega} \operatorname{grad}\bm{u}\operatorname{grad}\bm{v}\,dx
    - \int_{\Omega} p\,\operatorname{div}\bm{v}\,dx
    - \int_{\Omega} q\,\operatorname{div}\bm{u}\,dx
    = 0
    $$

    Для
    $$
    \bm{u}(\bm{x}) \in \bm{V_D}, \ \bm{v}(\bm{x}) \in \bm{V_0}, \ p(\bm{x}), q(\bm{x}) \in Q
    $$

    **В терминах функции тока** (2D):

    - Функция тока: $u_1 = \frac{\partial \psi}{\partial y},\quad u_2 = -\frac{\partial \psi}{\partial x}$
